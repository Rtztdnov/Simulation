...
public class Prey extends Creature{
    ...
    public Coordinates makeMove(Coordinates coordinates, WorldMap worldMap) {
        LinkedList<Coordinates> way = findTheWay(coordinates, worldMap);

        if (way.isEmpty()) {
            return coordinates;
        } else if (way.size() == SPEED + 1) {
            attack();
            return coordinates;
        }
        else {
            return way.get(SPEED);
        }
    }

    private LinkedList<Coordinates> findTheWay(Coordinates firstCoordinate, WorldMap worldMap) {
                ...
                if (!visited.contains(c)) {
                    if (!inheritorChain.containsKey(c)) {
                        ...
                        if (entitiesMap.containsKey(c)) {
                            if (entitiesMap.get(c) instanceof Grass) {
